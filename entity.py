...
class TestUserEntity(unittest.TestCase):
    def test_属性を変更する_成功(self):
        user = User(UserId(str(uuid.uuid4)), FullName("hoge", "taro"))
        user.change_full_name(FullName("piyo", "jiro"))

        self.assertEqual(FullName("piyo", "jiro"), user.full_name)

    def test_同じ属性でも区別される_成功(self):
        user1 = User(UserId(str(uuid.uuid4())), FullName("huga", "taro"))
        user2 = User(UserId(str(uuid.uuid4())), FullName("huga", "taro"))

        self.assertNotEqual(user1, user2)

    def test_属性が変わっても同一性が保証される_成功(self):
        user = User(UserId(str(uuid.uuid4())), FullName("huga", "taro"))
        before_user = user
